...
The `home-routes` handler in the `life-status-car.routes.home` namespace
...
The `home-page` function will in turn call the `life-status-car.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `life-status-car.handler` namespace:
...
Request middleware functions are located under the `life-status-car.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `life-status-car.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `life-status-car.db.core` in some other namespace.
